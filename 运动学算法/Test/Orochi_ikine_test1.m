...
% 定义目标点的空间位置
Px = 0.2*sin(pi*(j-1)/36); Py = 0.2*cos(pi*(j-1)/36); Pz = 0.5; %Pz=0.5+(j-1)/720;
p_f = [Px,Py,Pz];   
...
for n=1:73
    ...
    if D > 1e-2
        ...
        Px=0.2*sin(pi*(n-1)/36); Py=0.2*cos(pi*(n-1)/36); Pz=0.5; %Pz=0.5+(j-1)/720;
        D(n,:)=0;
        ...
        delta=pi/180;
        A = unifrnd(a(n,1)-delta,a(n,1)+delta,[1,100]);    % 构建q1变量邻域
        B = unifrnd(a(n,2)-delta,a(n,2)+delta,[1,100]);    % 构建q2变量邻域
        C = unifrnd(a(n,3)-delta,a(n,3)+delta,[1,100]);    % 构建q3变量邻域
        D = unifrnd(a(n,4)-delta,a(n,4)+delta,[1,100]);    % 构建q4变量邻域
        E = unifrnd(a(n,5)-delta,a(n,5)+delta,[1,100]);    % 构建q5变量邻域
        F = unifrnd(a(n,6)-delta,a(n,6)+delta,[1,100]);    % 构建q6变量邻域
        G = unifrnd(a(n,7)-delta,a(n,7)+delta,[1,100]);    % 构建q7变量邻域
        Q = cell(100,7);
...
Px=0.2*sin(pi*(s-1)/36); Py=0.2*cos(pi*(s-1)/36); Pz=0.5+0*s;       % 期望轨迹
% Pz=0.5+(j-1)/720;  
...
subplot(4,2,1)
plot(t,P_1(:,1),'o-')
xlabel('迭代次数'); ylabel('q1角度')
grid on

subplot(4,2,2)
plot(t,P_2(:,1),'o-')
xlabel('迭代次数'); ylabel('q2角度')
grid on

subplot(4,2,3)
plot(t,P_3(:,1),'o-')
xlabel('迭代次数'); ylabel('q3角度')
grid on

subplot(4,2,4)
plot(t,P_4(:,1),'o-')
xlabel('迭代次数'); ylabel('q4角度')
grid on

subplot(4,2,5)
plot(t,P_5(:,1),'o-')
xlabel('迭代次数'); ylabel('q5角度')
grid on

subplot(4,2,6)
plot(t,P_6(:,1),'o-')
xlabel('迭代次数'); ylabel('q6角度')
grid on

subplot(4,2,7)
plot(t,P_7(:,1),'o-')
xlabel('迭代次数'); ylabel('q7角度')
grid on
